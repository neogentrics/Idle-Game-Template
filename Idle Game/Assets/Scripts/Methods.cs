...
public static class Methods
{
    ...
    public static string Notate(this BigDouble number, int normalDigits = 0, int mantissaDigits = 3)
    {
        // Todo: Show full number till neg/pos 1 million is reached
        ...
        switch (Notation)
        {
            case 0: //Standard Notation
                string numberString = (number / BigDouble.Pow(10, 3 * BigDouble.Floor(number.exponent / 3))).ToString($"F{mantissaDigits}"); // Goal is to get 145,760,000 => 145.760M
                string abbreviationString = number.exponent < 303 ? StandardNotation[(int)((number.exponent -(number.exponent % 3)) / 3)]
            ...
            case 2: //Engineering Notation
                return $"{(number / BigDouble.Pow(10, 3 * BigDouble.Floor(number.exponent / 3))).ToString($"F{mantissaDigits}")}e{number.exponent - number.exponent % 3:N0}";
            case 3: //Logarithmic Notation
                return $"e{BigDouble.Log10(BigDouble.Abs(number)).ToString($"F{mantissaDigits}")}";
        }
        return "";
    }
    ...
    public static void UpgradeCheck<T>(List<T> list, int length) where T : new()
    {
        ...
        catch { list = new T[length].ToList(); }
    }
